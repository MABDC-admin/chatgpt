...
# Nexum Router is a flat-rate subscription ($5.50/week), so per-image "cost"
# does not correspond to any real spend. We bill a small nominal amount to the
# user's allocation so heavy Qwen use still shows up in usage reports and
# cannot silently blow through fair-use; the admin absorbs the actual flat fee
# separately. Set to 0 to make Qwen unmetered.
NEXUM_FLAT_COST_CENTS = 1


def _is_nexum_model(model: str) -> bool:
    """True for anything routed through Nexum. Uses a prefix rule so that
    aspirational names ('qwen-image-3.0-pro') still bill at the flat Nexum
    rate rather than falling through to gpt-image-1 pricing."""
...
def image_cost_cents(model: str, quality: str, size: str, count: int = 1) -> int:
    ...
    Nexum-routed models (Qwen) are subscription-priced upstream, so per-image
    "cost" is a nominal fair-use marker rather than a real charge.
    """
    if _is_nexum_model(model):
        return NEXUM_FLAT_COST_CENTS * max(count, 1)
...
def image_cost_from_usage(model: str, usage) -> int | None:
    ...
    """
    # Nexum-routed models keep the flat nominal charge.
    if _is_nexum_model(model):
        return NEXUM_FLAT_COST_CENTS
