...
def _supports_response_format(model: str) -> bool:
    """gpt-image-2.5-* models reject the `response_format` parameter entirely.
    ...
    gpt-image-2.5-flare return 400 BadRequestError when they see it. We must
    explicitly omit the parameter for those models while keeping it for older
    ones (gpt-image-1, gpt-image-1-mini) that require it.
    """
    return not model.startswith("gpt-image-2.5")
...
def default_model(requested: str | None = None) -> str:
    ...
    return requested if requested in IMAGE_TOKEN_PRICES else settings.image_model
